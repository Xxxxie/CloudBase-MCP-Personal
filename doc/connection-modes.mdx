...
| `TCB_AUTH_OAUTH_CUSTOM` | 自定义 endpoint 是否使用 toolbox custom 返回格式（高级可选） | 默认 `false`；仅当返回格式与默认协议不一致时才需要 |
...
- 服务器、CI、远程环境：优先配 `TENCENTCLOUD_SECRETID`、`TENCENTCLOUD_SECRETKEY`、`TENCENTCLOUD_SESSIONTOKEN`、`CLOUDBASE_ENV_ID`
...
- `TCB_AUTH_OAUTH_CUSTOM`：只有自定义 endpoint 返回格式不是默认协议时才设为 `true`
...
当前提供约 32 个 MCP 工具，按插件分布如下。完整参数与说明见 [MCP 工具](/ai/cloudbase-ai-toolkit/mcp-tools)。
